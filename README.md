...
<p align="center">
  <a href="https://dongshuangcheng.github.io/kiwi-trader-showcase/">打开交互演示 ↗</a> ·
  <a href="#项目概览">项目概览</a> ·
  <a href="#系统架构">系统架构</a> ·
...
### [↗ 打开 Kiwi Trader 交互作品集](https://dongshuangcheng.github.io/kiwi-trader-showcase/)

这是一个可操作的工程展示页，不是静态截图，也不是真实交易终端。

| 体验 | 可以操作什么 | 看见什么设计 |
| --- | --- | --- |
| **决策回放** | 单步或自动回放；切换正常、超时、行情过期场景 | 同一份快照如何经过研究、风控、意图与回执 |
| **异常对账** | 重复请求；补充合成证据；重置演示 | 超时保留 unknown，重复请求不产生第二次模拟提交 |
| **数据闸门** | 拖动快照年龄滑块 | 旧数据继续展示，执行资格独立判断 |
| **交互架构** | 点击六个职责模块 | 每层负责什么，以及它刻意避开的权限 |
| **工程案例** | 展开三个案例 | 一致性、精度与恢复能力背后的取舍 |

**建议体验路径：** 先回放正常流程 → 选择「行情过期」看拦截 → 打开「异常对账」重复请求 → 补充证据，观察提交次数保持不变。

全部使用合成标识与演示参数；零真实行情、零账户连接、零业务 API 请求。桌面和手机均可访问。
...
> **这是面向作品集与简历的公开介绍仓库。** 收录重新整理的项目说明、架构图与独立编写的前端合成演示，不含核心业务源码、运行数据或可连接真实环境的配置。项目使用模拟交易，不展示投资收益或实盘效果。
...
![系统架构：研究、受控执行、账本与只读展示](assets/architecture.svg)
...
- 本仓库：项目概览、架构图、工程取舍、纯前端合成演示与其测试。
- 保留在私有环境：业务实现、策略参数、提示词、测试明细、运行记录和部署配置。
- 不公开：账户与持仓、订单与成交、资金与收益、凭据、私有地址、本机路径及原仓库历史。

此仓库不是交易服务，没有真实系统的运行入口。详见 [公开范围与隐私说明](docs/privacy.md)。

## 展示页的设计与验证

参考公开作品集的信息组织方式，页面与交互独立实现。参考来源和本地检查方式见 [展示设计说明](docs/design-notes.md)。
